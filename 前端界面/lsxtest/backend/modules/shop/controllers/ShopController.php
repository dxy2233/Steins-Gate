    ...
    public function actionAdd()
    {
        ...
        if(posts())
        {
            ...
            try
            {
                //存储营业执照
                $img = '';
                if($_FILES['business']['size']){
                    $img = Upload::UploadPhoto('','uploads/business/','business');
                    $img = str_replace('uploads/','',$img);
                    $post['business_license_path']=$img;

                }
                //存卫生许可证
                $img = '';
                if($_FILES['hygienic']['size']){
                    $img = Upload::UploadPhoto('','uploads/hygienic/','hygienic');
                    $img = str_replace('uploads/','',$img);
                    $post['food_license_path']=$img;
                }
                if(empty($post['business_license_path'])||empty($post['shop_name']))
                {
                    throw new Exception('关键数据为空，不能保存');
                }
//            var_dump($post);die();
...
//        var_dump($level);die();
    ...
    public function actionInfo()
    {
        ...
        $data['business_license_path']=$data['business_license_path'] ? imageurl($data['business_license_path']) : '';
        $data['food_license_path']=$data['food_license_path'] ? imageurl($data['food_license_path']) : '';
            ...
            if(posts())
            {
               $post=posts();
                ...
                if($post['shop_status']==0)
                {
                    ...
                    ShopBanquet::closeShopbanquet($shop_id);
