...
<div class="box">
    <div class="imglogo"><img src="{if !empty($lsx_info.banquet_url)}{imageurl($lsx_info.banquet_url)}{else}/img/xiaolongxia.png{/if}"/></div>
    ...
    <!--已拼满-->
        <div class="xq_renshu">
            <div class="xq_age1">
                <span class="dinshi">{$seat_details.eat_time}<label> 开席</label></span>
                <span class="xq_ren_name">{$seat_details.banquet_title} </span>
                {if $seat_details.is_display eq 0}<span class="mui-icon mui-icon-locked" style="margin-left: 3%;font-weight: 500"></span>{/if}
                <span class="renhsugershu">
			 	 		<i class="renshuicon"><img src="/img/renshuimg_03.png"/></i>
			 	 		<span><label>{$seat_details.joined_peoples}</label>/<label>{$seat_details.total_peoples}</label> </span>
			 	 	</span>
            </div>
            <div class="renshutoux">
			 	 	<span class="iconrsu">
                        {foreach from=$seat_details.user_info item=user_info key=key}
                            {if $key lte 4}
            ...
            <div class="xq_lijiruxi">
                <span class="zuohoa">桌号：<label>{$seat_details.banquet_number}</label></span>
                {if $seat_details.joined_peoples eq $seat_details.total_peoples}
                    <a href="javascript:void(0);" class="lijiruxixq">
                        {if $seat_details.order_status eq 3}
                            <span style="font-size: 0.1rem;color: red">已完成</span>
                        {else}
                            <img src="/img/yipinman.png"/>
                        {/if}
                    </a>
                {/if}
                {if $seat_details.joined_peoples neq $seat_details.total_peoples}
                        <a href="/checkout/checkout/join?order_id={$seat_details.order_id}&banquet_id={$seat_details.banquet_id}{if $seat_details.is_display eq 0}&sign={$sign}{/if}" class="lijiruxixq"><img src="/img/lijiruxi.png"/></a>
                {/if}
            </div>
        </div>
...
<script>
    $(function(){
        ...
        $('.kaixi').click(function(){
            console.log({$lsx_info.left_number});
            console.log({$lsx_info.left_number_2});
//            return false;
            if( {$lsx_info.left_number} <= 0  && {$lsx_info.left_number_2} > 0 ){
                //信息框
//                layer.open({
//                    content: '只能开明天的了？'
//                    ,btn: '我知道了'
...
//                        location.reload();
                ...
                return false;
            }else if({$lsx_info.left_number} <= 0  && {$lsx_info.left_number_2} <= 0){
                //信息框
                layer.open({
