...
class Cls_http
{
	...
	public function https_post($url, $data, $signature)
	{
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_HEADER, 0);
		curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/json; charset=utf-8', 'accept: application/json', 'Authorization: Basic ' . $signature));
	...
	public function http_get($url, $token = '')
	{
		...
		if (!empty($token)) {
			...
			curl_setopt($oCurl, CURLOPT_HTTPHEADER, array('Content-Type: application/json; charset=utf-8', 'accept: application/json', 'Authorization: bearer ' . $token));
		}
