...
superagent
  ...
  .then(async res => {
    ...
    // let arrayData = [
    //   ['用户编号', '购房者姓名', '购房者手机号', '置业顾问', '通话状态', '客户产生时间', '客户来源', '操作', '更新时间', '音频地址', '通话时长',  '跟进记录', '意向', '通话记录']
    // ]
    // let resObj = {
    //   id: '',
    //   name: '',
    ...
    $('#dateListTable tbody tr').each((i, elem) => {
      ...
      let temp = {
        ...
        phone: $(elem).children().eq(3).text(),
        adviser: $(elem).children().eq(4).text(),
        callStatus: $(elem).children().eq(5).text(),
        createTime: $(elem).children().eq(6).text(),
        origin: $(elem).children().eq(7).text(),
        operateUrl: $(elem).children().eq(8).children().attr('href'),
        updateTime: now.getMonth() + 1 + '/' + now.getDate(),
        audioUrl: '',
        callTime: '',
        log: '',
        intent: '',
